...
<div class="max-w-5xl mx-auto px-4 py-10">
    ...
    <div class="card p-5 mb-8 flex flex-col sm:flex-row items-start gap-5">
        <img src="<?php echo e($service->image_url); ?>" alt="<?php echo e($service->name); ?>" class="w-24 h-24 rounded-xl object-cover flex-shrink-0">
        <div class="flex-1">
            <h1 class="text-xl font-bold text-gray-900 dark:text-white mb-1"><?php echo e($service->name); ?></h1>
            <p class="text-sm text-gray-500">Category: <span class="text-gray-700 dark:text-gray-300"><?php echo e($service->category->name); ?></span>
            <?php if($service->subCategory): ?> › <?php echo e($service->subCategory->name); ?> <?php endif; ?></p>
            <div class="flex items-center gap-2 mt-1">
                <img src="<?php echo e($service->provider->profile_image_url); ?>" class="w-5 h-5 rounded-full" alt="">
                <span class="text-sm text-gray-500"><?php echo e($service->provider->full_name); ?></span>
                ...
                <span class="text-xs text-gray-400"><?php echo e($service->avg_rating); ?> (<?php echo e($service->total_reviews); ?>)</span>
            </div>
        </div>
        <div class="text-right">
            <p class="text-2xl font-bold text-primary-600">
                <?php if($service->price == 0): ?> Free
                <?php else: ?> $<?php echo e(number_format($service->discounted_price, 2)); ?>

                <?php endif; ?>
            </p>
            <?php if($service->duration): ?>
            <p class="text-xs text-gray-400">/ <?php echo e($service->duration); ?> min</p>
            <?php endif; ?>
        </div>
    </div>

    <form method="POST" action="<?php echo e(route('customer.booking.store', $service)); ?>" id="bookingForm">
        <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            ...
                            <textarea name="address" rows="2" required placeholder="e.g. 72 Elite Street, Union Square, San Francisco, CA 94102"
                                      class="form-input pl-10" id="addressInput"><?php echo e(old('address')); ?></textarea>
                        </div>
                    ...
                    <div class="mb-2">
                        ...
                        <textarea name="notes" rows="2" placeholder="Any special instructions..."
                                  class="form-input"><?php echo e(old('notes')); ?></textarea>
                    </div>
                </div>

                
                <?php if($service->addons->count()): ?>
                <div class="card p-6">
                    <h3 class="font-bold text-gray-900 dark:text-white mb-4">Add-ons (Optional)</h3>
                    <div class="space-y-3">
                        <?php $__currentLoopData = $service->addons->where('status','active'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $addon): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="flex items-center justify-between cursor-pointer p-3 border border-gray-100 dark:border-gray-700 rounded-xl hover:border-primary-300 transition-colors">
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="addons[]" value="<?php echo e($addon->id); ?>"
                                       class="rounded text-primary-600 addon-check" data-price="<?php echo e($addon->price); ?>">
                                <div class="flex items-center gap-2">
                                    <?php if($addon->image): ?>
                                    <img src="<?php echo e(asset('storage/'.$addon->image)); ?>" class="w-8 h-8 rounded-lg object-cover" alt="">
                                    <?php endif; ?>
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300"><?php echo e($addon->name); ?></span>
                                </div>
                            </div>
                            <span class="font-bold text-primary-600 text-sm">+$<?php echo e(number_format($addon->price,2)); ?></span>
                        </label>
                ...
                <div class="card p-6">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <?php $__currentLoopData = ['cash'=>'fas fa-money-bill-wave','stripe'=>'fab fa-stripe-s','razorpay'=>'fas fa-rupee-sign','wallet'=>'fas fa-wallet']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $method => $icon): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="cursor-pointer">
                            <input type="radio" name="payment_type" value="<?php echo e($method); ?>" class="hidden peer" <?php echo e($method==='cash'?'checked':''); ?>>
                            <div class="flex items-center gap-2 p-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl peer-checked:border-primary-500 peer-checked:bg-primary-50 dark:peer-checked:bg-primary-900/20 transition-all">
                                <i class="<?php echo e($icon); ?> text-primary-600"></i>
                                <span class="text-sm font-medium capitalize text-gray-700 dark:text-gray-300"><?php echo e(ucfirst($method)); ?></span>
                            </div>
                        </label>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                ...
                <div class="card p-6 sticky top-24">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium" id="basePrice">$<?php echo e(number_format($service->price,2)); ?></span>
                        </div>
                        <?php if($service->discount > 0): ?>
                        <div class="flex justify-between text-green-600">
                            <span>Discount (<?php echo e($service->discount); ?>% Off)</span>
                            <span id="discountAmt">-$<?php echo e(number_format($service->price - $service->discounted_price, 2)); ?></span>
                        </div>
                        <?php endif; ?>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between text-gray-600 dark:text-gray-400">
                            <span>Subtotal</span>
                            <span id="subtotal">$<?php echo e(number_format($service->discounted_price,2)); ?></span>
                        </div>
                        ...
                        <div class="flex justify-between pt-3 border-t border-gray-100 dark:border-gray-700 font-bold text-base text-gray-900 dark:text-white">
                            <span>Total</span>
                            <span id="total">$<?php echo e(number_format($service->discounted_price,2)); ?></span>
                        </div>
                    </div>

                    <div class="flex gap-3 mt-6">
                        <a href="<?php echo e(route('service.detail',$service)); ?>" class="btn-secondary flex-1 justify-center">Cancel</a>
                        <?php if(auth()->guard()->check()): ?>
                        <button type="submit" class="btn-primary flex-1 justify-center">
                            <i class="fas fa-calendar-check"></i> Book Now
                        </button>
                        <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="btn-primary flex-1 justify-center">
...
<script>
let qty = 1;
const basePrice = <?php echo e($service->discounted_price); ?>;
const origPrice = <?php echo e($service->price); ?>;
...
function applyCoupon(code) {
    if(!code) return;
    fetch('<?php echo e(route("customer.coupon.apply")); ?>', {
        method: 'POST',
        headers: {'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>','Content-Type':'application/json','Accept':'application/json'},
        body: JSON.stringify({code, service_id: <?php echo e($service->id); ?>, amount: basePrice * qty})
    })
...
<?php echo $__env->make('layouts.public.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Downloads\handyman-platform (1) (1)\handyman\resources\views/customer/bookings/create.blade.php ENDPATH**/ ?>