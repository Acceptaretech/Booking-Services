...
<head>
    ...
    <title>@yield('title', 'Dashboard') | @yield('panel_name', 'Admin') Panel</title>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside id="sidebar" class="bg-gray-950 flex flex-col overflow-hidden flex-shrink-0">
            ...
            @if($cs = auth()->user()->commissionSetting ?? null)
                <div class="sb-commission mt-1.5 text-[11px] text-gray-500 pl-0.5">
                    ...
                    <span class="text-primary-400 font-semibold">
                        {{ $cs->commission_value }}{{ $cs->commission_type === 'percent' ? '%' : ' fixed' }}
                    </span>
                    <span class="text-gray-600">· {{ ucfirst($cs->commission_type) }}</span>
                </div>
            @endif
        </div>
        ...
        <nav class="flex-1 overflow-y-auto py-2 px-2 space-y-0.5" id="sidebar-nav">
            ...
            <a href="{{ route('admin.bookings.index') }}" class="sb-link {{ request()->routeIs('admin.bookings*') ? 'active' : '' }}">
                ...
                @if($pendingBookings > 0)
                    <span class="sb-badge">{{ $pendingBookings }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('admin.sub-categories.index') }}" class="sb-link {{ request()->routeIs('admin.sub-categories*') ? 'active' : '' }}">
            ...
            <div>
                <button type="button"
                        ...
                        class="sb-link justify-between {{ $serviceMenuOpen ? 'active' : '' }}">
                    ...
                    <i id="services-arrow" class="fas fa-chevron-down text-xs sb-arrow transition-transform {{ $serviceMenuOpen ? 'rotate-180' : '' }}"></i>
                </button>

                <div id="services-menu" class="sb-submenu {{ $serviceMenuOpen ? '' : 'hidden' }}">
                    ...
                    <a href="{{ route('admin.addons.index') }}" class="sb-link {{ request()->routeIs('admin.addons*') ? 'active' : '' }}">
                        <i class="fas fa-plus-square sb-icon"></i>
                        <span class="sb-label">Addons</span>
                    </a>

                    @if(Route::has('admin.service-requests.index'))
                        <a href="{{ route('admin.service-requests.index') }}" class="sb-link {{ request()->routeIs('admin.service-requests*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.zones.index') }}" class="sb-link {{ request()->routeIs('admin.zones*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.jobs.index') }}" class="sb-link {{ request()->routeIs('admin.jobs.index') ? 'active' : '' }}">
                <i class="fas fa-clipboard-list sb-icon"></i>
                <span class="sb-label">Job Request List</span>
            </a>

            <a href="{{ route('admin.jobs.services') }}" class="sb-link {{ request()->routeIs('admin.jobs.services') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.shops.index') }}"
               class="sb-link {{ request()->routeIs('admin.shops*') ? 'active' : '' }}"
               title="Shops">
            ...
            <div>
                <button type="button"
                        ...
                        class="sb-link justify-between {{ $providerMenuOpen ? 'active' : '' }}">
                    ...
                    <i id="provider-arrow" class="fas fa-chevron-down text-xs sb-arrow transition-transform {{ $providerMenuOpen ? 'rotate-180' : '' }}"></i>
                </button>

                <div id="provider-menu" class="sb-submenu {{ $providerMenuOpen ? '' : 'hidden' }}">

                    <a href="{{ route('admin.providers.index') }}"
                       class="sb-link {{ request()->routeIs('admin.providers.index') ? 'active' : '' }}">
                        <i class="fas fa-list sb-icon"></i>
                        <span class="sb-label">Provider List</span>
                    </a>
                
                    <a href="{{ route('admin.providers.requests') }}"
                       class="sb-link {{ request()->routeIs('admin.providers.requests') ? 'active' : '' }}">
                        <i class="fas fa-clipboard-list sb-icon"></i>
                        <span class="sb-label">Provider Request List</span>
                    </a>
                
                    <a href="{{ route('admin.providers.commissions') }}"
                       class="sb-link {{ request()->routeIs('admin.providers.commissions') ? 'active' : '' }}">
            ...
            <div>
                <button type="button"
                        ...
                        class="sb-link justify-between {{ $handymanMenuOpen ? 'active' : '' }}">
                ...
                <div id="handyman-menu" class="sb-submenu {{ $handymanMenuOpen ? '' : 'hidden' }}">
            ...
            {{-- Unverified Users --}}
            <a href="{{ route('admin.unverified-users.index') }}"
            class="sb-link {{ request()->routeIs('admin.unverified-users*') ? 'active' : '' }}">
           ...
           <a href="{{ route('admin.payments.cash') }}"
            ...
            <a href="{{ route('admin.banners.index') }}" class="sb-link {{ request()->routeIs('admin.banners*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.blogs.index') }}"
                class="sb-link {{ request()->routeIs('admin.blogs*') ? 'active' : '' }}">
            ...
            <div>
                <button type="button"
                        ...
                        class="sb-link justify-between {{ $pagesMenuOpen ? 'active' : '' }}">
                    ...
                    <i id="pages-arrow"
                    class="fas fa-chevron-down text-xs sb-arrow transition-transform {{ $pagesMenuOpen ? 'rotate-180' : '' }}">
                    </i>
                </button>

                <div id="pages-menu" class="sb-submenu {{ $pagesMenuOpen ? '' : 'hidden' }}">

                    <a href="{{ route('admin.pages.edit', 'about-us') }}"
                    class="sb-link {{ request()->is('admin/pages/about-us*') ? 'active' : '' }}">
                        <i class="fas fa-info-circle sb-icon"></i>
                        <span class="sb-label">About Us</span>
                    </a>

                    <a href="{{ route('admin.pages.edit', 'terms-and-conditions') }}"
                    class="sb-link {{ request()->is('admin/pages/terms-and-conditions*') ? 'active' : '' }}">
                    ...
                    class="sb-link {{ request()->is('admin/pages/privacy-policy*') ? 'active' : '' }}">
                        <i class="fas fa-shield-alt sb-icon"></i>
                        <span class="sb-label">Privacy Policy</span>
                    </a>

                    <a href="{{ route('admin.pages.edit', 'help-and-support') }}"
                    class="sb-link {{ request()->is('admin/pages/help-and-support*') ? 'active' : '' }}">
                        <i class="fas fa-question-circle sb-icon"></i>
                        <span class="sb-label">Help And Support</span>
                    </a>

                    <a href="{{ route('admin.pages.edit', 'refund-and-cancellation-policy') }}"
                    class="sb-link {{ request()->is('admin/pages/refund-and-cancellation-policy*') ? 'active' : '' }}">
            ...
            <div>
                <button type="button"
                        ...
                        class="sb-link justify-between {{ $documentsMenuOpen ? 'active' : '' }}">
                    ...
                    class="fas fa-chevron-down text-xs sb-arrow transition-transform {{ $documentsMenuOpen ? 'rotate-180' : '' }}">
                    </i>
                </button>

                <div id="documents-menu" class="sb-submenu {{ $documentsMenuOpen ? '' : 'hidden' }}">

                    <a href="{{ route('admin.documents.index') }}"
                    class="sb-link {{ request()->routeIs('admin.documents.index') ? 'active' : '' }}">
                        <i class="far fa-clipboard sb-icon"></i>
                        <span class="sb-label">Document List</span>
                    </a>

                    <a href="{{ route('admin.documents.create') }}"
                    class="sb-link {{ request()->routeIs('admin.documents.create') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.help-desk.index') }}" class="sb-link {{ request()->routeIs('admin.help-desk*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.settings.general') }}" class="sb-link {{ request()->routeIs('admin.settings*') ? 'active' : '' }}">
            ...
            <div class="flex items-center gap-1.5">
                ...
                <div class="relative group ml-1">
                    <button class="flex items-center gap-2 pl-1 pr-2 py-1 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                        ...
                        <div class="hidden sm:block text-left">
                            <p class="text-xs font-semibold text-gray-800 dark:text-white leading-tight">
                                {{ Str::limit(auth()->user()->full_name ?? auth()->user()->name ?? 'Admin',18) }}
                            </p>
                    ...
                    <div class="absolute right-0 top-11 w-52 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                        ...
                        <div class="py-1">
                            <a href="{{ route('home') }}" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <i class="fas fa-home w-4 text-gray-400 text-xs"></i>
                                Home
                            </a>

                            <a href="{{ Route::has('admin.settings.generalx') ? route('admin.settings.general') : '#' }}" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
