                            ...
                            <td class="p-5">
                                <div class="flex items-center gap-3">
                                    <div class="w-11 h-11 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold">
                                        {{ strtoupper(substr($payment->customer->first_name ?? $payment->customer->firstname ?? $payment->customer->name ?? 'U', 0, 1)) }}
                                    </div>

                                    <div>
                                        <p class="font-bold text-gray-900 dark:text-white">
                                            {{ trim(($payment->customer->first_name ?? $payment->customer->firstname ?? '') . ' ' . ($payment->customer->last_name ?? $payment->customer->lastname ?? '')) ?: ($payment->customer->name ?? 'No User') }}
                                        </p>
